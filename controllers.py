...
def listar(request: Request, db: Session = Depends(get_db)):
    ...
    # Pega todos os produtos da tabela Produto
    produtos = db.query(Produto).all()

    return templates.TemplateResponse("loja.html", {
        "request": request,
        "usuario": usuario,
        "produtos": produtos   # envia todos os produtos para o template
    })
...
def ver_produto(produto_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    comentarios = db.query(Comentario).filter(Comentario.produto_id == produto_id).all()
    print("OOOOoooooooo")
    for c in comentarios:
        print(c.usuario, c.rating, c.comentario)
